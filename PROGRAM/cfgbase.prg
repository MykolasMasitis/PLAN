...
IF FILE('plan.cfg')
 USE plan.cfg IN 0 ALIAS cnfg EXCLUSIVE 
 SELECT cnfg
...
IF MESSAGEBOX('ОТСУТСТВУЕТ КОНФИГУРАЦИОННЫЙ ФАЙЛ PLAN.CFG!' + CHR(13) + 'СОЗДАТЬ?',4+32,'ВНИМАНИЕ!') = 6
	CREATE TABLE plan ;
	...
	SELECT plan
	USE 
	RENAME  plan.dbf TO plan.cfg
	INSERT INTO plan.cfg (qcod, tyear, tmonth, tdat1, tdat2, user, Frmt, paisoms, parc, pbase, pbin, pcommon, ;
	                         pout, plocal, pexpimp, pmail, ptempl, ptrash, pdouble, pmail) ;
		...
		GOMONTH(CTOD('01.'+PADL(MONTH(DATE()),2,'0')+'.'+PADL(YEAR(DATE()),4,'0')),1)-1, 'OMS', 'DOC', ;
		 ...
		 m.HomePath+'TRASH', m.HomePath+'DOUBLES', m.HomePath+'MAIL')
	SELECT plan
ELSE
